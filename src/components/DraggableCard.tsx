...
import { SanGuoShaCardContent } from './sanguosha/SanGuoShaCardContent';
...
const DraggableCard: React.FC<DraggableCardProps> = ({
  ...
  const borderStyle = isSelected ? { borderColor: '#A0A0A0', borderWidth: 2 } : { borderColor: '#E0E0E0', borderWidth: 1 };
  ...
  const renderCardContent = () => {
    if (gameName === 'UnoLite') {
      ...
      return <SanGuoShaCardContent card={card} />;
    } else {
  ...
  return (
    ...
    >
      <TouchableOpacity activeOpacity={0.8} accessibilityRole="button" onPress={() => onPress(index)} disabled={isLocked}>
        <Animated.View style={{
          transform: [
            { translateY: selectAnim.interpolate({ inputRange: [0, 1], outputRange: [0, -10] }) }
          ]
        }}>
          <View
            style={[
